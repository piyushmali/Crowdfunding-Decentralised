...
describe('Crowdfunding', function () {
  ...
  describe('Creator', function () {
    it('should create a project', async function () {
      ...
      const projectDetails = await Crowdfunding.getProjectDetails(projectIndex)
      ...
      expect(projectDetails.deadline).to.equal(
        (await time.latest()) + Duration_In_Sec
      )
    ...
    it('should fail when invalid project index given to get project details', async function () {
      ...
      await expect(
        Crowdfunding.connect(creator).getProjectDetails(projectIndex)
    ...
    it('should fail when invalid project index given to withdraw raised fund', async function () {
      const projectIndex = 1
      await expect(
        Crowdfunding.connect(creator).withdrawFunds(projectIndex)
      ).to.be.revertedWith('Invalid project index')
    })
    ...
    it('should fail when non-creator try to withdraw raised fund', async function () {
      const projectIndex = 0
      await expect(
        Crowdfunding.connect(contributor1).withdrawFunds(projectIndex)
    ...
    it('should withdraw raised fund when the project is completed Successfully', async function () {
      ...
      await Crowdfunding.connect(contributor1).contribute(projectIndex, {
        value: contributionAmount,
      })

      await hre.network.provider.send('evm_increaseTime', [New_Duration_In_Sec])

      //await Crowdfunding.connect(creator).withdrawFunds(projectIndex);
      await expect(Crowdfunding.connect(creator).withdrawFunds(projectIndex))
        .to.emit(Crowdfunding, 'Withdrawed')
        .withArgs(
          projectIndex,
          creator.address,
          contributionAmount
  ...
  describe('Contributor', function () {
    it('should contibute to a project', async function () {
      ...
      await expect(
        Crowdfunding.connect(contributor1).contribute(projectIndex, {
          value: contributionAmount,
        })
      )
    ...
    it('should be able to claim refund if the project is failed', async function () {
      ...
      await expect(Crowdfunding.connect(contributor1).claimRefund(projectIndex))
    ...
    it('should not be able to claim refund if the project index is invalid ', async function () {
      ...
      await expect(
        Crowdfunding.connect(contributor1).claimRefund(newProjectIndex)
      ).to.be.revertedWith('Invalid project index')
    ...
    it('should not be able to claim refund if the project is Active ', async function () {
      ...
      await expect(
        Crowdfunding.connect(contributor1).claimRefund(projectIndex)
    ...
    it('should not be able to claim refund if not contributed to the project ', async function () {
      ...
      await expect(
        Crowdfunding.connect(contributor2).claimRefund(projectIndex)
